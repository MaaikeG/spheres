...
class PassThroughBoundaries(BoundaryConditionHandler):
    ...
    def _handle_boundary_conditions_1d(self, data, d):
        data.qs[:, d] = np.where(data.qs[:, d] > self.boundaries[d][1], self.boundaries[d][0], data.qs[:, d])
        data.qs[:, d] = np.where(data.qs[:, d] < self.boundaries[d][0], self.boundaries[d][1], data.qs[:, d])
        return data
